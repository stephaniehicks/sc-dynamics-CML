---
title: Analysis of scRNA-seq data from CML patient 0222
...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, 
                      message = FALSE)
library(here)
data_path <- "/users/shicks1/data/10X/sc-dynamics-cml/All_patient_data/0222"
```

# Summary of patient data

This is scRNA-seq (10X Genomics platform) data from a MPN patient 
(denoted `0222` patient). The data is available 
...
dat_muts_illumina <- readr::read_csv(file.path(data_path, 
                                      "Python_AnnData_NS_100_All_Cells_0222_v2.csv"))
...
# save SCE object
saveRDS(sce, file = file.path(data_path, "sce_0222.rds"))
```
...
library(SingleCellExperiment)
library(scater)
sce <- readRDS(file.path(data_path, "sce_0222.rds"))
```
...
saveRDS(sce, file = file.path(data_path, "sce_qc_0222.rds"))
```
...
sce <- readRDS(file.path(data_path, "sce_qc_0222.rds"))
```
...
```{r}
pdf(here::here("figures/2019-07-22-cell-cycle-0222.pdf"), width=12,height=4)
cowplot::plot_grid(p1, p2, ncol=2, labels = LETTERS[1:2])
dev.off()
```
...
```{r}
sce <- normalize(sce)
saveRDS(sce, file = file.path(data_path, "sce_norm_0222.rds"))
```
...
```{r}
sce_norm <- readRDS(file.path(data_path, "sce_norm_0222.rds"))
