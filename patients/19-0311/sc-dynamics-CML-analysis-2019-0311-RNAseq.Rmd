---
title: Analysis of scRNA-seq data from CML patient 0311
...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, 
                      message = FALSE)
library(here)
data_path <- "/users/shicks1/data/10X/sc-dynamics-cml/All_patient_data/0311"
```

# Summary of patient data

This is scRNA-seq (10X Genomics platform) data from a MPN patient 
(denoted `0311` patient). The data is available 
...
dat_muts_illumina <- readr::read_csv(file.path(data_path, 
                                      "Python_AnnData_NS_100_All_Cells_0311.csv"))
...
# save SCE object
saveRDS(sce, file = file.path(data_path, "sce_0311.rds"))
```
...
library(SingleCellExperiment)
library(scater)
sce <- readRDS(file.path(data_path, "sce_0311.rds"))
```
...
saveRDS(sce, file = file.path(data_path, "sce_qc_0311.rds"))
```
...
sce <- readRDS(file.path(data_path, "sce_qc_0311.rds"))
```
...
```{r}
pdf(here::here("figures/2019-07-22-cell-cycle-0311.pdf"), width=12,height=4)
cowplot::plot_grid(p1, p2, ncol=2, labels = LETTERS[1:2])
dev.off()
```
...
```{r}
sce <- normalize(sce)
saveRDS(sce, file = file.path(data_path, "sce_norm_0311.rds"))
```
...
```{r}
sce_norm <- readRDS(file.path(data_path, "sce_norm_0311.rds"))
