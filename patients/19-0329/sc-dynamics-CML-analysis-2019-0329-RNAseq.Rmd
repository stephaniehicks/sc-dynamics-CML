---
title: Analysis of scRNA-seq data from CML patient 0329
...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, 
                      message = FALSE)
library(here)
data_path <- "/users/shicks1/data/10X/sc-dynamics-cml/All_patient_data/0329"
```

# Summary of patient data

This is scRNA-seq (10X Genomics platform) data from a MPN patient 
(denoted `0329` patient) from Jan 14, 2019. The data is available 
...
dat_muts_illumina <- readr::read_csv(file.path(data_path, 
                                      "Python_AnnData_NS_100_All_Cells_0329.csv"))
colnames(dat_muts_illumina) <- c("jak2_illumina_wt", "jak2_illumina_cancer")

# calls from Illumina for tet2
dat_muts_tet2 <- readr::read_csv(file.path(data_path,
                                      "Python_AnnData_NS_100_All_Cells_0329_Tet2B.csv"))
colnames(dat_muts_tet2) <- c("tet2_illumina_wt", "tet2_illumina_cancer")

colData(sce) <- cbind(colData(sce), dat_muts_illumina, dat_muts_tet2)
```
...
# save SCE object
saveRDS(sce, file = file.path(data_path, "sce_0329.rds"))
```
...
library(SingleCellExperiment)
library(scater)
sce <- readRDS(file.path(data_path, "sce_0329.rds"))
```
...
table(sce$tet2_illumina_cancer, sce$tet2_illumina_wt)
```

Here is the table of mutant (jak2 and tet2) calls from 
illumina 

```{r}
table(sce$jak2_illumina_cancer, sce$tet2_illumina_cancer)
```
...
```{r}
sce$cancer_status_jak2 <- ifelse(sce$jak2_illumina_cancer > 0, "mut", 
  ifelse((sce$jak2_illumina_cancer < 1 & sce$jak2_illumina_wt > 0), "wt", "unknown" ))
sce$cancer_status_jak2 <- factor(sce$cancer_status_jak2, 
                                     levels=c("wt", "mut", "unknown"))
table(sce$cancer_status_jak2)

sce$cancer_status_tet2 <- ifelse(sce$tet2_illumina_cancer > 0, "mut", 
  ifelse((sce$tet2_illumina_cancer < 1 & sce$tet2_illumina_wt > 0), "wt", "unknown" ))
sce$cancer_status_tet2 <- factor(sce$cancer_status_tet2, 
                                   levels=c("wt", "mut", "unknown"))
table(sce$cancer_status_tet2)
```

Here is the able of agreement between the two types of calls
```{r}
table(sce$cancer_status_jak2, sce$cancer_status_tet2)
```
...
boxplot(sce$total_counts ~ sce$cancer_status_jak2, log="y", 
        main = "cancer_status_jak2", ylab = "total_counts")
...
boxplot(sce$total_features_by_counts ~ sce$cancer_status_jak2, log="y", 
        main = "cancer_status_jak2", ylab = "total_features_by_counts")
```

This is with the tet2 calls

```{r, fig.height=8, fig.width=12}
par(mfrow=c(2,3))
boxplot(sce$total_counts ~ sce$tet2_illumina_cancer, log="y", 
        main = "tet2_illumina_cancer", ylab = "total_counts" )
boxplot(sce$total_counts ~ sce$tet2_illumina_wt, log="y", 
        main = "tet2_illumina_wt", ylab = "total_counts")
boxplot(sce$total_counts ~ sce$cancer_status_tet2, log="y", 
        main = "cancer_status_tet2", ylab = "total_counts")

boxplot(sce$total_features_by_counts ~ sce$tet2_illumina_cancer, log="y", 
        main = "tet2_illumina_cancer", ylab = "total_features_by_counts" )
boxplot(sce$total_features_by_counts ~ sce$tet2_illumina_wt, log="y", 
        main = "tet2_illumina_wt", ylab = "total_features_by_counts")
boxplot(sce$total_features_by_counts ~ sce$cancer_status_tet2, log="y", 
        main = "cancer_status_tet2", ylab = "total_features_by_counts")
```
...
par(mfrow=c(2,2))
boxplot(sce$pct_counts_MT ~ sce$cancer_status_jak2, ylim = c(0, 50), 
        ylab = "pct_counts_MT", main = "cancer_status_jak2")
plot(sce$total_features_by_counts, sce$pct_counts_MT)

boxplot(sce$pct_counts_MT ~ sce$cancer_status_tet2, ylim = c(0, 50), 
        ylab = "pct_counts_MT", main = "cancer_status_tet2")
plot(sce$total_features_by_counts, sce$pct_counts_MT)
```
...
saveRDS(sce, file = file.path(data_path, "sce_qc_0329.rds"))
```
...
sce <- readRDS(file.path(data_path, "sce_qc_0329.rds"))
```
...
```{r}
pdf(here::here("figures/2019-07-22-cell-cycle-0329.pdf"), width=12,height=4)
cowplot::plot_grid(p1, p2, ncol=2, labels = LETTERS[1:2])
dev.off()
```
...
x <- table(factor(sce$cancer_status_jak2[sce$cancer_status_jak2 != "unknown"]), 
      sce$phases[sce$cancer_status_jak2 != "unknown"]) 
x
...
```{r}
x <- table(factor(sce$cancer_status_tet2[sce$cancer_status_tet2 != "unknown"]), 
      sce$phases[sce$cancer_status_tet2 != "unknown"]) 
x
...
```{r}
sce <- normalize(sce)
saveRDS(sce, file = file.path(data_path, "sce_norm_0329.rds"))
```
...
```{r}
sce_norm <- readRDS(file.path(data_path, "sce_norm_0329.rds"))
...
plot_hexbin_meta(sce_norm, col="cancer_status_jak2", action="prop")
...
saveRDS(monocle_cds, file = file.path(data_path, "cds_monocle_0329.rds"))
rm(monocle_cds)
```
...
```{r}
library(monocle)
monocle_cds <- readRDS(file = file.path(data_path, "cds_monocle_0329.rds"))
...
plot_cell_trajectory(monocle_cds, color_by = "cancer_status_jak2", cell_size = 0.5)
plot_cell_trajectory(monocle_cds, color_by = "cancer_status_tet2", cell_size = 0.5)
...
x <- table(factor(monocle_cds$cancer_status_jak2[monocle_cds$cancer_status_jak2 != "unknown"]), 
      monocle_cds$phases[monocle_cds$cancer_status_jak2 != "unknown"]) 
x
...
x <- table(factor(monocle_cds$cancer_status_tet2[monocle_cds$cancer_status_tet2 != "unknown"]), 
      monocle_cds$phases[monocle_cds$cancer_status_tet2 != "unknown"]) 
x
...
frac_cancer_jak2 <- sapply(levels(xx), function(i){
  tmp <- monocle_cds$cancer_status_jak2[xx == i &  !(monocle_cds$cancer_status_jak2 == "unknown")]
  2*sum(tmp == "mut", na.rm = TRUE) / sum(tmp %in% c("wt", "mut"), na.rm = TRUE)
})
frac_cancer_jak2


frac_cancer_tet2 <- sapply(levels(xx), function(i){
  tmp <- monocle_cds$cancer_status_tet2[xx == i &  !(monocle_cds$cancer_status_tet2 == "unknown")]
  2*sum(tmp == "mut", na.rm = TRUE) / sum(tmp %in% c("wt", "mut"), na.rm = TRUE)
})
frac_cancer_tet2
