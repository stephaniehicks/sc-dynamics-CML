...
```{r umap-G}
plotUMAP(sce_sub, colour_by="MPO") + ggtitle("umap (G -- MPO)") # G
```
...
pd <- as.data.frame(colData(sce_sub))
fd <- as.data.frame(rowData(sce_sub))
...
plot_cell_clusters(monocle_cds, color_by = "Cluster")
...
monocle_cds2 <- setOrderingFilter(monocle_cds, ordering_genes$gene_id)
table(fData(monocle_cds2)$use_for_ordering)

monocle_cds2 <- reduceDimension(monocle_cds2, max_components = 2, 
                               reduction_method = 'DDRTree') # this takes a while.... 
monocle_cds2 <- orderCells(monocle_cds2)
dim(monocle_cds2)

plot_cell_trajectory(monocle_cds2, color_by = "Cluster", cell_size = 0.5)
plot_cell_trajectory(monocle_cds2, color_by = "Pseudotime", cell_size = 0.5)
plot_cell_trajectory(monocle_cds2, color_by = "State", cell_size = 0.5)
plot_cell_trajectory(monocle_cds2, color_by = "cancer_status", cell_size = 0.5)
plot_cell_trajectory(monocle_cds2, color_by = "phases", cell_size = 0.5)
plot_cell_trajectory(monocle_cds2, color_by = "cellcycle_score_normalized_G1", cell_size = 0.5)
plot_cell_trajectory(monocle_cds2, color_by = "cellcycle_score_normalized_G2M", cell_size = 0.5)
plot_cell_trajectory(monocle_cds2, color_by = "cellcycle_score_normalized_S", cell_size = 0.5)

pData(monocle_cds2)$CD34 <- ifelse(exprs(monocle_cds2)[grep("CD34", rownames(monocle_cds2)), ]>0, 1, 0)
plot_cell_trajectory(monocle_cds2, color_by = "as.factor(CD34)", cell_size = 0.5)

## Multi-Potent Progenitor/MPP
pData(monocle_cds2)$HLF <- ifelse(exprs(monocle_cds2)[grep("^HLF", rownames(monocle_cds2)), ]>0, 1, 0)
plot_cell_trajectory(monocle_cds2, color_by = "as.factor(HLF)", cell_size = 0.5) 

## Granulocyte/macrophage progenitors/GMP
pData(monocle_cds2)$MPO <- ifelse(exprs(monocle_cds2)[grep("^MPO", rownames(monocle_cds2)), ]>0, 1, 0)
plot_cell_trajectory(monocle_cds2, color_by = "as.factor(MPO)", cell_size = 0.5) 

## MEP 
pData(monocle_cds2)$ITGA2B <- ifelse(exprs(monocle_cds2)[grep("^ITGA2B", rownames(monocle_cds2)), ]>0, 1, 0)
plot_cell_trajectory(monocle_cds2, color_by = "as.factor(ITGA2B)", cell_size = 0.5) # Megakaryocytes/MK

pData(monocle_cds2)$HBB <- ifelse(exprs(monocle_cds2)[grep("^HBB", rownames(monocle_cds2)), ]>0, 1, 0)
plot_cell_trajectory(monocle_cds2, color_by = "as.factor(HBB)", cell_size = 0.5) # Erythrocytes/E

pData(monocle_cds2)$EPOR <- ifelse(exprs(monocle_cds2)[grep("^EPOR", rownames(monocle_cds2)), ]>0, 1, 0)
plot_cell_trajectory(monocle_cds2, color_by = "as.factor(EPOR)", cell_size = 0.5) # Erythrocytes/E

pData(monocle_cds2)$LMO4 <- ifelse(exprs(monocle_cds2)[grep("^LMO4", rownames(monocle_cds2)), ]>0, 1, 0)
plot_cell_trajectory(monocle_cds2, color_by = "as.factor(LMO4)", cell_size = 0.5) # Ba

pData(monocle_cds2)$CD79A <- ifelse(exprs(monocle_cds2)[grep("^CD79A", rownames(monocle_cds2)), ]>0, 1, 0)
plot_cell_trajectory(monocle_cds2, color_by = "as.factor(CD79A)", cell_size = 0.5) # Ly

pData(monocle_cds2)$CSF1R <- ifelse(exprs(monocle_cds2)[grep("^CSF1R", rownames(monocle_cds2)), ]>0, 1, 0)
plot_cell_trajectory(monocle_cds2, color_by = "as.factor(CSF1R)", cell_size = 0.5) # M
...
```{r}
table(monocle_cds2$phases)

x <- table(factor(monocle_cds2$cancer_status[monocle_cds2$cancer_status != "unknown"]), 
      monocle_cds2$phases[monocle_cds2$cancer_status != "unknown"]) 
x
...
summary(monocle_cds2$Pseudotime)
xx <- cut(monocle_cds2$Pseudotime, breaks = seq(0, max(monocle_cds2$Pseudotime) + 3, by = 3))

n_mut <- sapply(levels(xx), function(i){
  tmp <- monocle_cds2$cancer_status[xx == i &  !(monocle_cds2$cancer_status == "unknown")]
