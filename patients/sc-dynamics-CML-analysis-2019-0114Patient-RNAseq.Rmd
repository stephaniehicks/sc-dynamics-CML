...
- `0114/barcodes.tsv`: barcode sequences of the detected cells.
- `0114/features.tsv`: names of the genes (columns of the count matrix)
- `0114/matrix.mtx`: count matrix (445.8 MB in size). The rows are the cells and the columns are the genes (9931 cells, 33694 genes). Each entry is the number of detected transcripts of a given gene in a given cell.
...
```{r}
pdf(here::here("figures/2019-07-22-cell-cycle.pdf"), width=12,height=4)
cowplot::plot_grid(p1, p2, ncol=2, labels = LETTERS[1:2])
dev.off()
```
...
keep <- which(rowSums(as.matrix(counts(sce)[1:10,]) >= num_reads ) >= floor(num_cells))
sce <- sce[keep,]
sce
```
