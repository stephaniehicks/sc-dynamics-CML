...
We see the p-value is 0.002513, which is statistically
...
First let's look at which genes are the most highly expressed
```{r}
...
(or -2 on `log10` scale). We also remove genes without 
...
```{r}
num.cells <- nexprs(sce_norm, byrow=TRUE)
keep_feature <- (num.cells > 5) 

sce2 <- sce_norm[keep_feature,]

var.fit <- trendVar(sce2, method = "spline", 
                    parametric=TRUE, weighted = FALSE)#,
                    #loess.args=list(span=0.5))
